/**************************************************************************//**
 * @file     hid_usb.c
 * @brief    M487 HSUSBD HID gamepad. IRQ path from Nuvoton HSUSBD_HID_Mouse.
 *
...
{
    static uint8_t u8Report[HID_REPORT_SIZE];
    ...
    if (gUsbCmd.bmRequestType & 0x80) { // Device to host
        ...
        case GET_REPORT:
            HSUSBD_PrepareCtrlIn(u8Report, HID_REPORT_SIZE);
            HSUSBD_CLR_CEP_INT_FLAG(HSUSBD_CEPINTSTS_INTKIF_Msk);
            HSUSBD_ENABLE_CEP_INT(HSUSBD_CEPINTEN_INTKIEN_Msk);
            break;
        case GET_IDLE:
...
void HID_UpdateGamepadData(void)
{
    ...
    HSUSBD->EP[EPA].EPTXCNT = HID_REPORT_SIZE;
    g_u8EPAReady = 0;
