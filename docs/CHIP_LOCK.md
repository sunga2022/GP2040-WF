...
主机蓝牙若一定要做，**不要自己在 nRF52 上仿**。国内现成的是山弯科技 **BM769P** 交钥匙 IC：客户给的 `支持NS+PS4-BM769P_smp_v107_….exe` 是他们的 **HID 固件烧录器**（工程来自 `BM-729-ALL`，USB VID `2563` 山弯），不是通用蓝牙协议栈。芯片 + 他们的闭源固件才会以 Switch / DS4 形态出现在主机上。
