...
| 蓝牙连什么 | 行不行 | 原因 |
|---|---|---|
| Windows / macOS / Linux、安卓、iPhone | **行** | 标准 BLE HID over GATT |
| **Xbox** 主机 | **不行** | 只认自家 Xbox Wireless / 授权手柄，不认普通 BLE HID |
| **PS4 / PS5** | **不行** | DualShock/DualSense 走 **蓝牙经典（BR/EDR）** + 索尼私有描述符和认证。nRF52840 **没有经典蓝牙射频** |
| **Switch** | **不行** | Pro 手柄同样是 **经典蓝牙** + 任天堂 HID。nRF52840 做不了 |

要主机无线，得另换带经典蓝牙的方案（还要仿官方手柄协议，认证/封禁风险另算）。现在这套主机侧只考虑以后用 **有线 USB** 去做 XInput / Switch / PS 描述符，和 GP2040-CE 有线那条路一样，**蓝牙不负责主机。**
